...
거짓말 탐지를 위해서 뇌파, 심전도, 영상 및 음성 등 여러가지 모드를 합성하여 사용하고 있다. 다중 모드를 합성하여 거짓말 탐지를 할 때 모의 범죄 심문에서 79%의 정확도를 보여 단일 모드를 사용하는 것보다 더 높은 정확도를 보여준다. 그 중 비디오 모달리티는 모의 범죄 심문에서 96%의 정확도를 제공하기에 거짓말을 할 때 표정에서 큰 변화가 일어난다는 것을 알 수 있어 비디오 모달리티의 중요성이 높다. 다양한 모드들을 이용하고, 각 모달리티의 정확도를 높여 최적의 합성 방법으로 합성한다면 더 높은 정확도를 얻는 거짓말 탐지 프로그램으로 나아갈 수 있을 것이다.
